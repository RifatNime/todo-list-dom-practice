...
function addItem(){
    ...
    if(inputValue.value == ""){
        ...
        tr.innerHTML = `<td class="item-name">${inputValue.value}</td>
                        ...
                        <td class="actions">
                            <button class="btn btn-success me-2 complete">
                            <ion-icon name="checkmark-outline"></ion-icon></button>
                            <button class="btn btn-danger delete"><ion-icon name="trash-outline"></ion-icon></button>
                        </td>`;
        ...
        for(const button of completeButton){
            button.addEventListener('click', function(event){
                const trackTodoItem =event.target.parentNode.parentNode.parentNode.childNodes[0];
                trackTodoItem.style.textDecoration = 'line-through';
                trackTodoItem.style.color = 'rgb(25, 135, 83)';
                const trackStatus = event.target.parentNode.parentNode.parentNode.childNodes[2];
        ...
        for(const button of deleteButton){
            button.addEventListener('click', function(event){
                const trackTr = event.target.parentNode.parentNode.parentNode.parentNode;
            trackTr.removeChild(event.target.parentNode.parentNode.parentNode);
